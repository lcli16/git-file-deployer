...
脚本具备自动检测并补充生产环境中缺失文件的能力：

1. 在每次部署时，无论是否有文件变更，都会检查仓库中的所有文件
2. 对比生产环境目录，找出在仓库中存在但在生产环境中缺失的文件
3. 自动将这些缺失的文件从仓库同步到生产环境
4. 这一机制特别适用于首次部署或部分文件意外丢失的情况
...
脚本会自动将 `/static/css/style.css` 和 `/static/images/logo.png` 同步到生产环境，确保文件完整性。
...
7. **补充缺失文件**: 检查并同步生产环境中缺失的文件
8. **清理文件**: 删除已移除的文件
9. **更新版本**: 记录当前部署的版本号
10. **清理旧备份**: 保留指定数量的备份，删除旧备份
